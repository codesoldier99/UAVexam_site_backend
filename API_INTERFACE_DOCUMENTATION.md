...
### 3. 机构管理模块 (Institution Management) - 7个接口
...
本次更新新增了4个关键的微信小程序API接口：

1. **`GET /api/v1/wechat/candidate/info-by-idcard`** - 根据身份证获取考生信息
   - 用于登录前的身份验证
   - 支持完整的错误处理和用户引导

2. **`POST /api/v1/wechat/candidate/qrcode/refresh`** - 二维码刷新功能
   - 支持频率限制和安全控制
   - 提供详细的刷新状态反馈

3. **`GET /api/v1/wechat/candidate/checkin-history`** - 考生签到历史查询
   - 支持分页、筛选和统计功能
   - 提供完整的签到记录追踪

4. **`GET /api/v1/wechat/candidate/exam-results`** - 考生考试结果查询
   - 支持多维度筛选和统计分析
   - 提供详细的考试成绩和通过率信息

这些接口完善了微信小程序的核心功能，为考生提供了完整的考试流程支持。
