...
def test_candidate_info_by_idcard():
    ...
    # 测试获取考生信息
    response = requests.get(f"{BASE_URL}/wechat/candidate/info-by-idcard", 
    ...
    if response.status_code == 200:
        ...
        print(f"❌ 身份证验证接口失败: {response.status_code}")
        print(response.text)
...
def test_checkin_new_format(examiner_token, qr_code_data):
    """测试新格式的签到接口"""
    print("\n=== 测试新格式签到接口 ===")
    
    if not qr_code_data:
        print("❌ 没有二维码数据，跳过签到测试")
        return
...
def main():
    ...
    if token:
        ...
        # 7. 测试新格式签到接口（需要考务人员token）
        # 这里假设有考务人员token，实际使用时需要先登录考务人员账号
        # examiner_token = "examiner_token_here"
        # test_checkin_new_format(examiner_token, qr_code_data)
